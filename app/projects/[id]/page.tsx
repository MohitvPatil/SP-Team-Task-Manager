...
export default function ProjectDetailsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const addComment = (e: React.FormEvent) => {
    ...
    // Attach to first task or just generic project
  ...
  const handleSave = (data: ProjectFormData) => {
    ...
    // Mutate sample data so it reflects on Dashboard if user navigates back
  ...
  const handleSaveMembers = (memberIds: string[]) => {
    const updatedMembers = buildMembers(project.id, memberIds);
    const updated = {
      ...project,
  ...
  return (
    <ProtectedRoute>
      ...
      {membersModal && <ManageMembersModal initialMemberIds={project.members.map(m => m.userId)} onSave={handleSaveMembers} onClose={() => setMembersModal(false)} />}
      {showDeleteConfirm && (
      ...
      <div className="min-h-screen bg-gray-50 dark:bg-gray-950 transition-colors">
        ...
        <main className="mx-auto max-w-5xl px-4 py-8 md:px-8">
          ...
          <div className="mb-8 rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-900 p-6 shadow-sm sm:p-8 transition-colors">
              ...
              {isLead && (
                <div className="flex items-center gap-2">
                  ...
                  <button
                    ...
                    className="rounded-lg border border-red-100 bg-white px-4 py-2 text-sm font-medium text-red-600 transition hover:bg-red-50"
                  >
                    Delete
                  </button>
                </div>
            ...
            <div className="mt-8 grid grid-cols-2 gap-6 border-t border-gray-100 pt-6 sm:grid-cols-4">
              <div>
                <p className="flex items-center gap-1.5 text-xs font-medium text-gray-500">
                  <Clock size={14} /> Start Date
                </p>
                <p className="mt-1.5 text-sm font-semibold text-gray-900">{fmt(project.assignedAt)}</p>
              </div>
              <div>
                <p className="flex items-center gap-1.5 text-xs font-medium text-gray-500">
                  <CalendarDays size={14} /> Deadline
                </p>
                <p className="mt-1.5 text-sm font-semibold text-gray-900">{fmt(project.deadline)}</p>
              </div>
              <div>
                <p className="flex items-center gap-1.5 text-xs font-medium text-gray-500">
                  <Users size={14} /> Team Size
                </p>
                <p className="mt-1.5 text-sm font-semibold text-gray-900">{project.members.length} members</p>
              </div>
              <div>
                <p className="flex items-center gap-1.5 text-xs font-medium text-gray-500">
                  Progress
            ...
            <div className="lg:col-span-2 space-y-8">
              ...
              <section className="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-900 p-6 shadow-sm transition-colors">
                <div className="mb-4 flex items-center justify-between">
                  ...
                  {isLead && (
                    <button onClick={() => setTaskModal(true)} className="flex items-center gap-1 text-xs font-medium text-blue-600 hover:text-blue-700">
                  ...
                  <div className="space-y-3">
                    {tasks.map((task) => {
                      ...
                      return (
                        <div
                          key={task.id}
                          className="flex items-start justify-between gap-4 rounded-xl border border-gray-100 p-4 transition hover:border-gray-200 hover:bg-gray-50"
                        >
                          <div className="flex items-start gap-3">
                            <button 
                              onClick={() => canClose && toggleTask(task.id)}
                              disabled={!canClose}
                              className={`mt-0.5 shrink-0 transition ${canClose ? "cursor-pointer hover:scale-110" : "cursor-not-allowed opacity-50"}`}
                              title={canClose ? "Toggle Task Status" : "Only assigned members or leads can toggle"}
                            >
                          ...
                          <div>
                            ...
                            <div className="mt-1.5 flex flex-wrap items-center gap-3 text-xs text-gray-500">
                              <span className={`font-medium ${task.priority === "URGENT" || task.priority === "HIGH" ? "text-red-600" : ""}`}>
                        ...
                        {/* Assignee */}
                        {task.assignedTo && (
                          <div className="flex shrink-0 items-center gap-2 rounded-full border border-gray-100 bg-white px-2 py-1">
                            <div className="flex h-5 w-5 items-center justify-center rounded-full bg-gray-900 text-[10px] font-bold text-white">
                              {task.assignedTo.name.charAt(0)}
                            </div>
                            <span className="text-xs font-medium text-gray-700">{task.assignedTo.name.split(" ")[0]}</span>
                          </div>
                        )}
                      </div>
                      );
              ...
              <section className="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-900 p-6 shadow-sm transition-colors">
                ...
                <form onSubmit={addComment} className="mb-6 flex gap-3">
                  <input
                    ...
                    className="flex-1 rounded-lg border border-gray-200 px-3 py-2 text-sm outline-none focus:border-gray-800 focus:ring-1 focus:ring-gray-800"
                  />
                  <button
                    type="submit"
                ...
                {comments.length === 0 ? (
                  <div className="rounded-lg border border-dashed border-gray-200 py-8 text-center text-sm text-gray-400">
                    No updates or issues reported yet.
                  </div>
                ) : (
                  <div className="space-y-4">
                    {comments.map((comment) => (
                      <div key={comment.id} className="flex gap-4 border-b border-gray-100 pb-4 last:border-0 last:pb-0">
            ...
            <div className="space-y-8">
              <section className="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-900 p-6 shadow-sm transition-colors">
                <div className="mb-4 flex items-center justify-between">
                  ...
                  {isLead && (
                    <button onClick={() => setMembersModal(true)} className="flex items-center justify-center rounded-md p-1.5 text-gray-400 hover:bg-gray-100 hover:text-gray-900">
                  ...
                  {project.members.map((member) => (
                    <div key={member.id} className="flex items-center gap-3 rounded-lg border border-gray-100 p-3">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="truncate text-xs text-gray-500">{member.user.position}</p>
                      </div>
                      {member.role === "MANAGER" || member.role === "ADMIN" ? (
                        <span className="shrink-0 rounded bg-blue-50 px-2 py-0.5 text-[10px] font-bold tracking-wider text-blue-700">
                          LEAD
                        </span>
                      ) : null}
                    </div>
